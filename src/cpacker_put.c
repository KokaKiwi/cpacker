...
{
    char data[sizeof(n)];
    uint64_t dn = buf->mode == CPACKER_MODE_LITTLE ? htole64(n) : htobe64(n);
    size_t i;
...
{
    char data[sizeof(n)];
    uint64_t dn = buf->mode == CPACKER_MODE_LITTLE ? htole32(n) : htobe32(n);
    size_t i;
...
{
    char data[sizeof(n)];
    uint64_t dn = buf->mode == CPACKER_MODE_LITTLE ? htole16(n) : htobe16(n);
    size_t i;
